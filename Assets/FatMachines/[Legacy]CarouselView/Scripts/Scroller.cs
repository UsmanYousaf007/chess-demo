...
namespace FM.Legacy{
	public class Scroller : MonoBehaviour {
		...
		public void SetCurrentItem(RectTransform _selectedItem, int index, Scaler item, bool updateScroller){
			if(selectedItem != _selectedItem){
				...
				if (movingLeft)
				{
					item.left.localPosition = new Vector2(item.transform.localPosition.x - childRectTransform.rect.width, item.left.localPosition.y);
				}
				else
				{
					item.right.localPosition = new Vector2(item.transform.localPosition.x + childRectTransform.rect.width, item.right.localPosition.y);
				}

				if (updateScroller)
				{
					rectTransform.sizeDelta = new Vector2(childRectTransform.rect.width + rectTransform.rect.width, rectTransform.rect.height);
				}
			}
		}
		...
		void Update () {
			...
			movingLeft = mousePosition.x - lastMousePosition.x > 0;
