...
namespace HUF.Analytics.Implementation
{
    public class AnalyticsModel
    {
        ...
        public void CollectSensitiveData(bool consentStatus, params string[] serviceNames)
        {
            if (serviceNames.Length > 0)
            {
                HLogs.LogFormatDebug("[AnalyticsModel] Set consent: {0} for services: [{1}]", 
                                     consentStatus, string.Join(",", serviceNames));
                foreach (var serviceName in serviceNames)
                {
                    CollectSensitiveData(consentStatus, serviceName);
                }
            }
            else
            {
