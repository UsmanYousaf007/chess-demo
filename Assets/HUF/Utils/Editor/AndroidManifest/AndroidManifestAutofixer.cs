...
namespace HUF.Utils.Editor.AndroidManifest
{
    ...
    public class AndroidManifestAutoFixer : IPreprocessBuildWithReport
    {
        ...
        public static void ReplaceAndroidManifestKeys()
        {
            ...
            {
                var config =
                    (AndroidManifestKeysConfig)AssetDatabase.LoadAssetAtPath( configString,
                        typeof(AndroidManifestKeysConfig) );

                if ( config == null )
                {
                    HLog.LogWarning( new HLogPrefix( nameof(AndroidManifestAutoFixer) ),
                        $"Could not load config {configString}" );
                    continue;
                }
                ...
                else if ( GetAndroidManifestTemplateDataToCopy( config, out string templatePath, out string savePath ) )
                {
                    AndroidManifestKeyReplacer.CreateFinalManifest( config,
                        templatePath,
                        $"{savePath}/{HUFBuildAssetsResolver.MANIFEST_FULL_NAME}" );
                    HUFBuildAssetsResolver.CreateProjectPropertyFile( savePath );
                }
            }
        }

        static bool GetAndroidManifestTemplateDataToCopy( AndroidManifestKeysConfig config,
            ...
            out string savePath )
        {
            ...
            var manifests = HUFBuildAssetsResolver
                .GetHUFManifests( HUFBuildAssetsResolver.MANIFEST_TEMPLATE_SEARCH, config.PackageName ).ToList();
