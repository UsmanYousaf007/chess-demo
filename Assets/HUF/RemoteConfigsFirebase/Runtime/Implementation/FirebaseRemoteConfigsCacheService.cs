...
namespace HUF.RemoteConfigsFirebase.Implementation
{
    public class FirebaseRemoteConfigsCacheService
    {
        ...
        public FirebaseRemoteConfigsCacheService()
        {
            ...
            Debug.Log($"[{logPrefix}] Initializing firebase remote config cache service");
        ...
        public Dictionary<string, string> ReadAllConfigs()
        {
            ...
            {
                Debug.Log($"[{logPrefix}] No cached configs found");
                return configs;
            }

            Debug.Log($"[{logPrefix}] Found {cachedKeys.Value.Length} cached configs");
            ...
            foreach (var cachedKey in cachedKeys.Value)
            {
                ...
                {
                    Debug.LogWarning($"[{logPrefix}] Failed to get configs from cache: {exception.Message}");
                }
            }
