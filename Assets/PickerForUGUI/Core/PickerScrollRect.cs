...
namespace Picker
{
	...
	public class PickerScrollRect : ScrollRect
	{
        ...
        public int itemCount                        { get{ return m_pickerLayoutGroup ? m_pickerLayoutGroup.transform.childCount : 0; } }
        ...
        protected float GetMagnitudeOfScrollVelocity()
        {
            Vector2 v = velocity;
            return layout == RectTransform.Axis.Horizontal ? Mathf.Abs(v.x) : Mathf.Abs(v.y);
        }
		...
		protected void NotifySelectItem( PickerItem item )
		{
			...
			{
                if( m_whenToSendEvents == WhenToSendEvents.OnlyScrollStops )
                {
                    if (GetMagnitudeOfScrollVelocity() > 0.1f * selfRect.rect.size[(int)layout])
                    {
                        return;
                    }
                }
		...
		protected virtual void AutoScroll()
		{
			...
			if( m_autoScroll )
			{
				...
				if( anchoredPosition != m_targetPosition )
				{
					...
					{
                        //Ease Out Quart
                        float dt = m_autoScrollTimeCount / autoScrollSeconds - 1;
                        float t = dt;
                        ...
                        Vector2 move = m_fromPosition - m_targetPosition;
                        velocity = (-4f * (dt * dt * dt)) * move;
                        content.anchoredPosition = move * t + m_fromPosition;
                    }
					else
					{
        ...
        [SerializeField]
        protected WhenToSendEvents m_whenToSendEvents = WhenToSendEvents.Always;

        public WhenToSendEvents whenToSendEvents
        {
            get
            {
                return m_whenToSendEvents;
            }

            set
            {
                m_whenToSendEvents = value;
            }
        }
