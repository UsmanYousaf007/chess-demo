...
#region Item
	...
	{
        //Method A. Scroll with index
        if ( 0 <= scrollTargetItemIndex && scrollTargetItemIndex < scrollRect.itemCount )
            scrollRect.ScrollToItemAtIndex(scrollTargetItemIndex);

#if false
        //Method B. Scroll using PickerItem
        ...
        //Method C. Scroll using item position
        Transform item = layoutGroup.transform.GetChild(scrollTargetItemIndex);

        if( item != null )
		{
			Picker.PickerItem pickerItem = item.GetComponent<Picker.PickerItem>();
			scrollRect.ScrollTo( pickerItem.position );
		}
#endif
    }
...
#region Other
    ...
    public void SetAlways( bool on )
    {
        if( on )
        {
            scrollRect.whenToSendEvents = Picker.WhenToSendEvents.Always;
        }
    }

    public void SetWhenScrollingStops( bool on )
    {
        if (on)
        {
            scrollRect.whenToSendEvents = Picker.WhenToSendEvents.OnlyScrollStops;
        }
    }
