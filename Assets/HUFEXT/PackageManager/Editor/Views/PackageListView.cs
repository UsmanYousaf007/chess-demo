...
namespace HUFEXT.PackageManager.Editor.Views
{
    public class PackageListView : PackageManagerView
    {
        ...
        static readonly Color labelColor = new Color( 0.27f, 0.27f, 0.27f );

        readonly List<ListItem> baseItems = new List<ListItem>();
        readonly List<ListItem> currentItems = new List<ListItem>();
        readonly List<ListItem> unityItems = new List<ListItem>();
        ...
        void RefreshItems()
        {
            ...
            for ( int i = 0; i < baseItems.Count; ++i )
            {
                switch ( baseItems[i].Type )
                {
                    ...
                    {
                        var listItem = (Items.HPackageListItem)baseItems[i];

                        if ( ShouldDrawPackageItem( listItem.package ) )
                        {
                            currentItems.Add( baseItems[i] );
                        }
        ...
        bool ShouldDrawPackageItem( Models.PackageManifest manifest )
        {
            ...
            if ( !string.IsNullOrEmpty( window.state.searchText ) )
            {
                return manifest.name.IndexOf( window.state.searchText, StringComparison.OrdinalIgnoreCase ) > -1 ||
                       manifest.displayName.IndexOf( window.state.searchText, StringComparison.OrdinalIgnoreCase ) > -1;
            }
            ...
            switch ( window.state.categoryType )
            {
                ...
                case Models.PackageCategoryType.HUFEXT:
                    if ( !manifest.name.Contains( "hufext." ) )
                    {
                        return false;
                    }
                ...
                case Models.PackageCategoryType.SDK:
                    if ( !manifest.name.Contains( ".plugins." ) )
                    {
                        return false;
                    }
                ...
                case Models.PackageCategoryType.UNITY:
                    if ( !manifest.name.Contains( ".unity." ) )
                    {
                        return false;
                    }
