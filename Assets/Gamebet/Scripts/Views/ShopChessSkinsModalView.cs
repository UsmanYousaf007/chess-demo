...
/// @date 2018-02-15 09:49:15 UTC+05:00
...
{
    public class ShopChessSkinsModalView : View
    {
        ...
        public void UpdateView(ShopVO vo)
        {
            ...
            titleLabel.text = vo.shopSettings.skinShopItems[vo.activeShopItemId].displayName;
            tierLabel.text = vo.shopSettings.skinShopItems[vo.activeShopItemId].tier;
            tierLabel.color = textColorCache.GetShopItemsHeadingSeperatorColor(vo.shopSettings.skinShopItems[vo.activeShopItemId].tier);
            tierLabelLeftSeperatorImage.sprite = spriteCache.GetShopItemsHeadingLeftSeperator(vo.shopSettings.skinShopItems[vo.activeShopItemId].tier);
            tierLabelRightSeperatorImage.sprite = spriteCache.GetShopItemsHeadingRightSeperator(vo.shopSettings.skinShopItems[vo.activeShopItemId].tier);

            chessSkinImage.sprite = skinThumbsContainer.GetThumb(vo.activeShopItemId).preview;

            enoughBucksDialogueLabel.text = localizationService.Get(LocalizationKey.S_M_BUY_FOR_LABEL);
            enoughBucksBuyPriceLabel.text = vo.shopSettings.skinShopItems[vo.activeShopItemId].currency2Cost.ToString();

            notEnoughBucksDialogueLabel.text = localizationService.Get(LocalizationKey.S_M_NOT_ENOUGH_BUCKS_LABEL);
            notEnoughBucksPriceLabel.text = vo.shopSettings.skinShopItems[vo.activeShopItemId].currency2Cost.ToString();

            confirmationDialogueLabel.text = localizationService.Get(LocalizationKey.S_M_PURCHASE_CHESS_SKIN_FOR_LABEL,vo.shopSettings.skinShopItems[vo.activeShopItemId].currency2Cost.ToString());
            ...
            addToCollectionDialogueLabel.text = localizationService.Get(LocalizationKey.S_M_BOUGHT_CHESS_SKIN_LABEL);
            ...
            else if (vo.playerModel.currency2 >= vo.shopSettings.skinShopItems[vo.activeShopItemId].currency2Cost)
            {
