...
{
    public class GSSellForgeCardsRequest : GSRequest
    {
            ...
            Action<LogEventResponse> successCallback)
        {
            ...
            new LogEventRequest().SetEventKey(GSEventData.SellForgeCards.EVT_KEY)
                .SetEventAttribute(GSEventData.SellForgeCards.ATT_KEY_FORGE_CARD_KEY, consumeVGoodShortCode)
                .SetEventAttribute(GSEventData.SellForgeCards.ATT_KEY_QUANTITY, consumeVGoodQuantity)
        ...
        private void OnFailure(LogEventResponse response)
        {
            if (!isExpired)
            {
                DispatchResponse(BackendResult.SELL_FORGE_CARDS_FAILED);
            }
        }
