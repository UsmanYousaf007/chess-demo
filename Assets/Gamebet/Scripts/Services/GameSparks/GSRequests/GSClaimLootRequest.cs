...
{
    public class GSClaimLootRequest : GSRequest
    {
        private IPromise<BackendResult> promise = new Promise<BackendResult>();
        private Action<LogEventResponse> successCallback;

        public IPromise<BackendResult> Send(string key,
            Action<LogEventResponse> successCallback)
        {
            GSRequestSession.instance.AddRequest(this);
            this.successCallback = successCallback;
            new LogEventRequest().SetEventKey(GSEventData.ClaimLoot.EVT_KEY)
                .SetEventAttribute(GSEventData.ClaimLoot.ATT_KEY_CLAIM_KEY, key)
