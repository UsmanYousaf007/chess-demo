...
{
    public class GSClaimAdRewardRequest : GSRequest
    {
        ...
        public IPromise<BackendResult> Send(Action<LogEventResponse> successCallback)
        {
            ...
            new LogEventRequest().SetEventKey(GSEventData.ClaimAdReward.EVT_KEY)
        ...
        private void OnFailure(LogEventResponse response)
        {
            if (!isExpired)
            {
                DispatchResponse(BackendResult.CLAIM_AD_REWARD_REQUEST_FAILED);
            }
        }
