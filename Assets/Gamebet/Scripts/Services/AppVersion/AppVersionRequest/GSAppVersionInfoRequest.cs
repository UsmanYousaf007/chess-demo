...
{
    public class GSAppVersionInfoRequest : GSRequest
    {
        ...
        public IPromise<BackendResult> Send(Action<LogEventResponse> successCallback)
        {
            ...
            new LogEventRequest().SetEventKey(GSEventData.GetAppInfo.EVT_KEY)
        ...
        private void OnFailure(LogEventResponse response)
        {
            if (!isExpired)
            {
                DispatchResponse(BackendResult.GET_APP_VERSION_DATA_REQUEST_FAILED);
            }
        }
