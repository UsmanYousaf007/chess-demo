...
namespace Crosstales.OnlineCheck.Tool
{
    ...
    public class Proxy : MonoBehaviour
    {
        ...
        [Header("HTTP Proxy Settings")] [Tooltip("URL (without protocol) or IP of the proxy server.")]
        public string HTTPProxyURL;

        /// <summary>Port of the proxy server.</summary>
        [Tooltip("Port of the proxy server.")] [Range(0, 65535)]
        ...
        /// <summary>URL (without protocol) or IP of the proxy server.</summary>
        [Header("HTTPS Proxy Settings")] [Tooltip("URL (without protocol) or IP of the proxy server.")]
        public string HTTPSProxyURL;

        /// <summary>Port of the proxy server.</summary>
        [Tooltip("Port of the proxy server.")] [Range(0, 65535)]
        public int HTTPSProxyPort = 8443;
        ...
        [Header("Startup Behaviour")] [Tooltip("Enable the proxy on awake (default: false).")]
        ...
        public static void EnableHTTPProxy(string url, int port, string username = "", string password = "", string urlProtocol = "")
        {
            if (Util.Helper.isEditor || Util.Helper.isStandalonePlatform)
            {
                ...
                {
                    System.Environment.SetEnvironmentVariable(HTTPProxyEnvVar,
                        urlProtocol + username + ":" + password + "@" + url + ":" + port);
                }
                else
                {
        ...
        public static void EnableHTTPSProxy(string url, int port, string username = "", string password = "", string urlProtocol = "")
        {
            if (Util.Helper.isEditor || Util.Helper.isStandalonePlatform)
            {
                ...
                if (!string.IsNullOrEmpty(username) && !string.IsNullOrEmpty(password))
                {
                    System.Environment.SetEnvironmentVariable(HTTPSProxyEnvVar,
                        urlProtocol + username + ":" + password + "@" + url + ":" + port);
                }
                else
                {
