...
namespace ChessboardLib
{
    ...
    public abstract class Player
    {
        ...
        {
            // TurboLabz: Original function was was not working
            // so we wrote a new one below. We also have this return
            // false here to exit quickly since this function is clearly
            // broken.
            return false;
            //////////
            /*
        ...
        /// </returns>
        /// 
        public bool TLCanClaimMoveRepetitionDraw()
        {
            int repeatCount = 1;
            Move lastMove = Game.MoveHistory.Last;

            if (lastMove.Piece.Name == Piece.PieceNames.Pawn || lastMove.PieceCaptured != null)
            {
                return false;
            }
                
            if (lastMove.ThreefoldCheckFEN == "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq -")
            {
                ++repeatCount;
            }

            for (int i = 0; i < Game.MoveHistory.Count - 1; ++i)
            {
                Move compareMove = Game.MoveHistory[i];

                if (lastMove.ThreefoldCheckFEN == compareMove.ThreefoldCheckFEN)
                {
                    ++repeatCount;
                }
            }
