    ...
    public class Piece : IPieceTop
    {
        ...
        public void GenerateLegalMoves(Moves moves)
        {
            ...
            for (int intIndex = moves.Count - 1; intIndex >= 0; intIndex--)
            {
				...
				ChessboardLib.Move.Undo(moveUndo);
            }
        }
        ...
        public Move Move(Move.MoveNames moveName, Square square)
        {
			...
			if (moveName == ChessboardLib.Move.MoveNames.EnPassent)
            {
            ...
            switch (moveName)
            {
                case ChessboardLib.Move.MoveNames.CastleKingSide:
			...
			case ChessboardLib.Move.MoveNames.CastleQueenSide:
                    pieceRook = move.Piece.Player.Colour == Player.PlayerColourNames.White
                                    ? Board.GetPiece(0, 0)
                                    : Board.GetPiece(0, 7);
                    Board.HashCodeA ^= pieceRook.HashCodeA;
			...
			case ChessboardLib.Move.MoveNames.PawnPromotionQueen:
                    this.Promote(PieceNames.Queen);
                    break;

			case ChessboardLib.Move.MoveNames.PawnPromotionRook:
                    this.Promote(PieceNames.Rook);
                    break;

			case ChessboardLib.Move.MoveNames.PawnPromotionBishop:
                    this.Promote(PieceNames.Bishop);
                    break;

			case ChessboardLib.Move.MoveNames.PawnPromotionKnight:
                    this.Promote(PieceNames.Knight);
                    break;

			case ChessboardLib.Move.MoveNames.EnPassent:
            ...
            string threefoldCheckFEN = Fen.GetBoardPosition();
            List<string> fenElements = new List<string>(threefoldCheckFEN.Split(' '));
            fenElements.RemoveRange(fenElements.Count - 2, 2);
            string colorToPlay = fenElements[fenElements.Count - 3];
            colorToPlay = (colorToPlay == "w") ? "b" : "w";
            fenElements[fenElements.Count - 3] = colorToPlay;
            move.ThreefoldCheckFEN = string.Join(" ", fenElements.ToArray());

            // Save castling rights (used for threefold repeat checks)
            /*
            string castlingRights = "";
            PieceKing whiteKing = null;
            PieceKing blackKing = null;

            if (move.Piece.Player.Colour == Player.PlayerColourNames.White)
            {
                whiteKing = (PieceKing)move.Piece.Player.King.Top;
                blackKing = (PieceKing)move.Piece.Player.OpposingPlayer.King.Top;
            }
            else
            {
                blackKing = (PieceKing)move.Piece.Player.King.Top;
                whiteKing = (PieceKing)move.Piece.Player.OpposingPlayer.King.Top;
            }
            ...
            move.EnPassantTarget = Fen.FenGet4EnPassant();
            */
