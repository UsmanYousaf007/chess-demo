...
namespace ChessboardLib
{
    ...
    public class Move : IComparable
    {
        ...
        public Move(int turnNo, int lastMoveTurnNo, MoveNames moveName, Piece piece, Square from, Square to, Piece pieceCaptured, int pieceCapturedOrdinal, int score)
        {
            ...
            {
                this.FiftyMoveDrawCounter = Game.MoveHistory.Count > 0 ? Game.MoveHistory.Last.FiftyMoveDrawCounter + 1 : Game.FiftyMoveDrawBase + 1;
            }
        }
        ...
        /// <summary>
        /// TurboLabz version: Converted this to algebraic notation
        /// </summary>
        public string TLDescription
        {
            get
            {
                StringBuilder strbMove = new StringBuilder();
                ...
                default:
                    ...
                    if (this.PieceCaptured != null)
                    {
                        if (this.Piece.Name == Piece.PieceNames.Pawn)
                        {
                            strbMove.Append(this.From.FileName);
                        }
                    ...
                    if (this.Name == MoveNames.EnPassent)
                    {
                        strbMove.Append("e.p.");    
                    }
                ...
                Player opposingPlayer = this.Piece.Player.OpposingPlayer;

                if (opposingPlayer.IsInCheckMate)
                {
                    strbMove.Append((this.Piece.Player.Colour == Player.PlayerColourNames.White) ? "# 1-0" : "# 0-1");
                }
                ...
                else if (opposingPlayer.Status == Player.PlayerStatusNames.InStalemate ||
                         this.IsThreeMoveRepetition ||
        ...
        public string ThreefoldCheckFEN { get; set; }
