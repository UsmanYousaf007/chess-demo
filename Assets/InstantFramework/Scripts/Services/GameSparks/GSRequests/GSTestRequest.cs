...
/// @date 2016-09-20 17:05:01 UTC+05:00
/// 
/// @description
...
namespace TurboLabz.InstantFramework
{
    public class GSTestRequest : GSRequest
    {
        ...
        public IPromise<BackendResult> Send(string eventKey)
        {
            GSRequestSession.Instance.AddRequest(this);
            new LogEventRequest().SetEventKey(eventKey).Send(OnSuccess, OnFailure);
            return promise;
        }
        ...
        private void OnFailure(LogEventResponse response)
        {
            if (!isExpired)
            {
                DispatchResponse(BackendResult.SEND_TEST_REQUEST_FAILED);
            }
        }
